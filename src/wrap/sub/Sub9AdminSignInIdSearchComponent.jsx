...
export default function Sub9AdminSignInIdSearchComponent(){
    ...
    const onSubmitIdSearch=(e)=>{
        ...
        axios({
            url: 'http://pse990813.dothome.co.kr/greekday/greekday_admin_id_search_name_email_select.php',
            method: 'POST',
            data: formData
        })
        .then((res)=>{
            if(res.data === 0){
                confirmModalMethod('가입 시 입력하신 관리자 정보가 맞는지 다시 한번 확인해주세요');
            }
            else{
                confirmModalMethod('아이디를 찾았습니다.')
                navigate('/sub9AdminIdSearchResult',
                {                  
                    state: {
                        id: res.data.아이디,
    ...
    return (
        <div id='sub9'>
            <div className="container">
                <div className="title">
                    <h2 className='title-txt'>관리자 아이디 찾기</h2>
                </div>
