...
export default function Section2Component({initState}) {
    ...
    React.useEffect(()=>{
        if(localStorage.getItem('GREEKDAY_VIEW_PRODUCT') !== null){
            let result = JSON.parse(localStorage.getItem('GREEKDAY_VIEW_PRODUCT'));
            if(result.length > 0){         
                dispatch(recentlyViewProduct(result));
            }
        }
                ...
                <ul>     
                    {
                        ...
                        (
                            initState.product.map((item, idx)=>{      
                                return(                                                              
                                    <li className={`list list${idx+1}`} onClick={(e)=>onClickProduct(e, item, './images/intro/section2/')}>
                                        <div className="img-box">
                                            <a href="!#">
                                                <img src={`./images/intro/section2/${item.이미지}`} alt="" />
                                            </a>
                                        </div>
